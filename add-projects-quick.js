// Quick script that works with current DB schema
...
async function addProjects() {
  try {
    console.log('🔄 Connecting and adding projects...');
    ...
    const typeCheckResult = await prisma.$queryRaw`
      SELECT column_name 
      FROM information_schema.columns 
      WHERE table_name = 'Project' AND column_name = 'type'
    `.catch(() => []);
    const hasTypeColumn = Array.isArray(typeCheckResult) && typeCheckResult.length > 0;
    
    if (hasTypeColumn) {
      // Use Prisma client if type column exists
      ...
      // Fallback to raw SQL if type column doesn't exist
      await prisma.$executeRaw`
        ...
        VALUES 
          ('REACH Journal Hub', 
           ...
           NOW(), 
           NOW()),
          ('IIIHWS - Integrative Healthcare',
           'A world-class institutional portal for the International Institute of Integrative Healthcare and Wellness Sciences. Designed to bridge modern medical advances with traditional wisdom. Features include accredited program management (Diploma to Doctorate), a translational research ecosystem, and a national certification framework for wellness practitioners. Tech Highlight: Mobile-first responsive design using Tailwind CSS, focusing on accessibility and seamless academic navigation.',
           NULL,
           'https://integrative-care.org/',
           1,
           NOW(),
           NOW())
      `;
    ...
    console.log('   1. REACH Journal Hub (DEVELOPMENT)');
    console.log('   2. IIIHWS - Integrative Healthcare (DEVELOPMENT)');
    if (hasTypeColumn) {
      console.log('   3. KLIBS Portal (DESIGN - Figma Prototype)');
    }
    console.log('\n💡 Refresh your browser now to see them!');
  } catch (error) {
