// Seed script for projects including KLIBS
...
async function seedProjects() {
  try {
    ...
    // Project 3: KLIBS Portal (Design Phase)
    const klibsProject = await prisma.project.create({
      data: {
        title: "KLIBS Portal",
    ...
    console.log(`✅ Created: ${klibsProject.title} (ID: ${klibsProject.id})`);
    
    console.log('\n🎉 All projects seeded successfully!');
    console.log('\n📋 Summary:');
    console.log(`   1. ${reachProject.title} - ${reachProject.type}`);
    console.log(`   2. ${integrativeProject.title} - ${integrativeProject.type}`);
    console.log(`   3. ${klibsProject.title} - ${klibsProject.type}`);
    console.log('\n💡 Refresh your browser to see them!');
  } catch (error) {
    console.error('❌ Error seeding projects:', error.message);
    if (error.code === 'P2022') {
      console.error('   Schema mismatch detected. Run: npm run db:push');
    }
    process.exit(1);
  } finally {
